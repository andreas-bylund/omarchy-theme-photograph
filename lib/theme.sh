...
# theme_write_meta OUT_DIR SLUG NAME SCENES_JSON NOTES_JSON
theme_write_meta() {
  local out="$1" slug="$2" name="$3" scenes="$4" notes="$5" stock=false
  ...
  jq -n \
    ...
    --argjson scenes "$scenes" --argjson notes "$notes" \
    '{
